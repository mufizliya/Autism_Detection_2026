...
class Bubble:
    def __init__(self, width, height, min_radius, max_radius):
...
def show_intro(screen, background_img, char_img, width, height, char_h, char_w, font_speech):
...
class BubbleGameModule:
    ...
    def run(self, session):
        ...
        char_img = pygame.image.load('src/character.png')

        char_w = WIDTH // 2.5
        char_h = int(
            char_img.get_height() *
            (char_w / char_img.get_width())
        )
        ...
        while running:
            ...
            if (
                ...
                bubbles.append(Bubble(WIDTH, HEIGHT, min_radius, max_radius))
                last_bubble_time = pygame.time.get_ticks()
    ...
    def calculate_behavioral_phenotypes(self):
        ...
        phenotypes = {
            ...
            "attention_deficit": round(
                min(avg_reaction / 3, 1),
                2
            ),
    ...
    def show_end_screen(
        ...
        bx = (info.current_w - bw)//2
        by = (info.current_h - bh)//2
